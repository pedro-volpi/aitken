...
O binding de pausa é a linha-sentinela `p`/`pause` + Enter, tratada em `_ask_active`. Não há tecla solta a usar: o processo fica bloqueado dentro de `input()` e o readline engole tudo até o Enter — modo raw continua proibido sem pedido explícito. O cronômetro, porém, **corre continuamente** (pedido explícito do usuário, 2026-08-25): a `ClockRefresher` de `src/mentat/ui/refresh.py` é uma thread de fundo que a cada centésimo sobrescreve só a linha do relógio via DECSC/sobe-N/DECRC em uma única `write()`, sem tocar na linha de edição do readline. Ela só é criada em terminal interativo (`styled`) e só pinta armada — `_ask_active` arma imediatamente antes do `ask` (apenas com o cronômetro correndo) e desarma em `finally`, então nada concorre com feedback, resumo ou o contrato `input_fn` dos testes. A saudação estampa o atalho em ASCII art (`_print_pause_banner`: `figlet` + `lolcat -f`, mesma cadeia de degradação do `mac-awake` dos dotfiles), mantendo a linha de texto puro como fonte canônica do binding. Pausado, a conta **sai da tela** e a linha do cronômetro perde o *faint* e ganha `[PAUSADO]`; deixar o problema visível permitiria pausar, resolver sem pressão e retomar com uma latência falsa — é a mesma família de proteção do feedback que nunca revela a resposta.
...
- Toda mudança de código deve passar antes de commit:
  - `pytest` (atualmente 199 testes, todos devem passar)
